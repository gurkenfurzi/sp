const CACHE_NAME='studia-v132-shell';
const SHELL=['./','./index.html?v=132','./manifest.webmanifest','./icon-192-v109.png','./icon-512-v109.png','./apple-touch-icon-v109.png'];
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(r=>{const copy=r.clone();caches.open(CACHE_NAME).then(c=>c.put(event.request,copy));return r}).catch(()=>caches.match(event.request).then(r=>r||caches.match('./index.html?v=132'))));
    return;
  }
